{
  ...
  resolveRef(schema, root):: (
    ...
    if refSchemaInDefs != null then { schema: refSchemaInDefs, root: root }
      ...
      local resolvedRoot = (
        if path != '' && path != pid then this.remote(path, root) else root
      ) + (
      ...
      local pidPatch = if std.objectHasAll(refSchema, fieldPID) then { [fieldPID]: refSchema[fieldPID] } else {};
  ...
  walk(o, keyPath, defaults=null, pid=''):: (
