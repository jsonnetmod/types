...
local validatorFor(value, path) = {
  ...
  validate(s, originRoot):: (
    if s == null then
      ...
      local root = refutil.withIDs(schemafix.fix(originRoot));
      local schema = refutil.withPID(schemafix.fix(s), root);
        ...
        if field == fieldSchema then
          ...
          local dependencies() = (
            if dependency != null then
              local validateDeps(dependency) = ValidErr.allOf(
                std.map(
                  function(depField)
                    function()
              ...
              if std.isString(dependency) then
                // Single Property dependencies
                validateDeps([dependency])
              else if std.isArray(dependency) then
